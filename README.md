...
- **📖 マニュアル**: カテゴリ別に業務手順書を登録し、折りたたみ表示で閲覧。1件ずつ手入力する以外に、「📥 まとめてインポート（JSON）」からJSONを取り込んで一括登録もできます（詳細は下記「📥 JSONインポート」参照）
...
- **🎓 トレーニング管理（名称変更可）**: 研修・資格制度のステップを登録し、完了時に本人が手書きサインして進捗を記録。機能名は店舗ごとに⚙️設定から自由に変更できます（コード上に固有の制度名は含まれません。実際の名称や内容はご自身のFirebaseデータにのみ保存されます）。ステップも1件ずつ手入力する以外に、「📥 まとめてインポート（JSON）」から一括登録できます
...
## 📥 マニュアル・トレーニング項目のJSONインポート

紙のマニュアルや研修資料を1件ずつ手入力するのは大変なので、📖マニュアルと🎓トレーニング管理には「📥 まとめてインポート（JSON）」という一括登録機能があります。生成AI（Claude等）に紙の資料の写真やPDFを読み取らせてJSONを作らせ、それをアプリに貼り付けるだけで反映できます。

**使い方**

1. 📖マニュアル または 🎓トレーニング管理の画面を開き、「📥 まとめてインポート（JSON）」を開く
...
4. Claudeが返してきたJSON（コードブロックの中身）をコピーし、アプリの「JSONを直接貼り付け」欄に貼り付けるか、`.json` ファイルとして保存してから「JSONファイルを選択」でアップロードする
5. 「📥 インポート実行」を押すと、配列の要素が1件ずつマニュアル／トレーニングステップとして登録される

**JSON形式**

- 📖マニュアル: `[{ "category": "カテゴリ名", "title": "見出し", "body": "本文" }]`（`{ "manuals": [...] }` の形でラップしてもOK）。`title` が空の要素はスキップされます
- 🎓トレーニング管理: `[{ "title": "ステップ名", "storeTags": ["店舗名", ...] }]`（`{ "steps": [...] }` でラップしてもOK）。`storeTags` は省略・空配列で「全店共通」になります。店舗名を指定する場合は🔒オーナー設定の「🏪店舗一覧」に登録済みの名前と完全一致させてください（一致しない名前が含まれる場合はインポート後に警告が表示されますが、登録自体は行われます）

取り込みはFirestoreへの通常の追加（1件ずつの `add()`）として行われるため、既存データは上書きされず追記されます。重複して取り込んでしまった場合は、各項目の「削除」ボタンから個別に削除してください。
