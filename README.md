...
1つのFirebaseプロジェクトを複数店舗で共有し、店舗ごとにデータを緩やかに区分けできます（アクセス制限はかけず、「基本は自店舗、必要なら他店舗も見える」という前提の設計です）。⚙️設定の「🏪 店舗設定」で管理します。

- **店舗一覧**: オーナー（管理者）が店舗名を登録します（1行に1店舗）
- **現在の店舗**: 各端末で選択する「今この端末が属している店舗」。📝連絡ノート・🎒忘れ物管理台帳・💬チャットルーム・🧾店控え書類の保管は、この店舗に投稿され、通常はこの店舗の分だけ表示されます
- **🌐 他店舗の投稿・記録も表示する**: チェックを入れると、上記4機能で全店舗分をまとめて表示できます（閲覧のみの切り替えで、投稿先は常に「現在の店舗」のままです）
