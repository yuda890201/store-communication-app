...
(async () => {
  ...
  check('time select option count', 96, timeOptionCount);
  const timeValue = await page.locator('#lostTimeInput').inputValue();
  info('default time value (HH:MM, minute should be multiple of 15)', timeValue);
  ...
  info('lostItemNames after first add', settingsAfterFirst.lostItemNames);
  info('lostLocationNames after first add', settingsAfterFirst.lostLocationNames);
  ...
  info('datalist options after first add', datalistOptions);
  ...
  info('lostItemNames after reusing same title (should still have only 1 entry, no dup)', settingsAfterSecond.lostItemNames);
